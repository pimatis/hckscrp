...
type HackerNewsSubmitted struct {
	Rank     string
	Title    string
	URL      string
	Domain   string
...
func SubmittedScrape(username string, page int) {
	c := colly.NewCollector()

	var items []HackerNewsSubmitted
	var nextURL string

	c.OnHTML("tr.athing.submission", func(e *colly.HTMLElement) {
		item := HackerNewsSubmitted{}
	...
	var url string
	if page == 1 {
		url = fmt.Sprintf("https://news.ycombinator.com/submitted?id=%s", username)
	} else {
		url = fmt.Sprintf("https://news.ycombinator.com/submitted?id=%s&p=%d", username, page)
	}
